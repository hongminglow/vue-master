<template>
  <div
    class="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl mx-auto space-y-8 pb-12"
  >
    <div class="space-y-2">
      <h1 class="text-3xl font-bold text-white flex items-center gap-3">
        <span class="text-green-500">🔬</span> Advanced Vue Arsenal
      </h1>
      <p class="text-slate-400 text-lg">
        Go beyond the basics with Vue's unique built-in tools. No third-party
        libraries required.
      </p>
    </div>
    ...
    <!-- Transition -->
    <div
      class="flex flex-col rounded-xl border-2 border-slate-700/50 bg-[#0F172A] overflow-hidden shadow-lg"
    >
      <div
        class="bg-slate-800/80 p-5 border-b border-slate-700/50 flex items-center justify-between"
      >
        <h3 class="text-lg font-bold text-white flex items-center gap-2">
          <span class="text-emerald-400">✨</span> &lt;Transition&gt;
        </h3>
        <span
      ...
      <div class="p-6 md:flex gap-8 items-start">
        <div class="space-y-4 flex-1">
          <p class="text-slate-300">
            Built directly into Vue core. Provide entering/leaving CSS classes
            natively via perfectly-timed lifecycle hooks. Much simpler and zero
            dependencies compared to React Transition Group or Framer Motion.
          </p>
          <div
          ...
          >
            ...
            ><span class="text-purple-300">name</span>=<span
              class="text-green-300"
              >"slide-fade"</span
            ><span class="text-blue-300">&gt;</span><br />
            &nbsp;&nbsp;<span class="text-blue-300">&lt;div&nbsp;</span
            ><span class="text-purple-300">v-if</span>=<span
              class="text-green-300"
              >"showBox"</span
            ><span class="text-blue-300">&gt;</span>I animate!<span
              class="text-blue-300"
              >&lt;/div&gt;</span
            ><br />
            <span class="text-blue-300">&lt;/Transition&gt;</span>
    ...
    >
      ...
      >
        <h3 class="text-lg font-bold text-white flex items-center gap-2">
          <span class="text-blue-400">⚡</span> v-memo
        </h3>
        <span
          class="text-xs font-mono px-2 py-1 rounded bg-blue-500/10 border border-blue-500/20 text-blue-400"
          >Performance</span
        >
      </div>
      <div class="p-6 space-y-4">
        <p class="text-slate-300">
          Vue's native supercharged alternative to <code>React.memo</code>. When
          rendering massive lists, you explicitly tell Vue to cache the DOM node
          entirely, completely bypassing virtual DOM diffing unless the
          designated dependencies change. No higher-order component wrappers
          needed!
        </p>
        <div
        ...
        >
          ...
          <span
            class="text-white bg-green-500/20 px-1 rounded border border-green-500/30 font-bold"
            >v-memo="[item.id, item.updatedAt]"</span
          >&gt;<br />
          &nbsp;&nbsp;<span class="text-slate-500"
            >&lt;!-- Hundreds of nested nodes here --&gt;</span
          ><br />
          &lt;/div&gt;
        </div>
      </div>
    ...
    >
      ...
      >
        <h3 class="text-lg font-bold text-white flex items-center gap-2">
          <span class="text-purple-400">📦</span> Scoped Slots
        </h3>
        <span
          class="text-xs font-mono px-2 py-1 rounded bg-purple-500/10 border border-purple-500/20 text-purple-400"
          >Architecture</span
        >
      </div>
      <div class="p-6 md:flex gap-8 items-start">
        <div class="space-y-4 flex-1">
          <p class="text-slate-300">
            The elegant Vue equivalent to React's <code>render props</code>. A
            child logic component handles state encapsulation or data fetching,
            but lets the <i>parent</i> define exactly how the UI should look by
          ...
          >
            <span
              class="text-slate-500 text-xs block mb-2 uppercase tracking-wide font-sans font-bold"
              >Parent Accesses Data via #default:</span
            >
            <span class="text-blue-300">&lt;template&nbsp;</span
            ><span class="text-purple-300">#default</span>=<span
              class="text-green-300"
              >"{ items }"</span
            ><span class="text-blue-300">&gt;</span><br />
    ...
    >
      ...
      >
        <h3 class="text-lg font-bold text-white flex items-center gap-2">
          <span class="text-cyan-400">🔁</span> Dynamic Components
        </h3>
        <span
          class="text-xs font-mono px-2 py-1 rounded bg-cyan-500/10 border border-cyan-500/20 text-cyan-400"
          >Rendering</span
        >
      </div>
      <div class="p-6 md:flex gap-8 items-start">
        <div class="space-y-4 flex-1">
          <p class="text-slate-300">
            Instead of massive <code>if/else</code> chains to display different
            components, Vue offers the generic
            <code>&lt;component :is="..."&gt;</code> element. It instantly
            renders whichever component object (or resolved string name) is
            bound to it!
          </p>
          <div
            class="bg-slate-900 rounded-lg p-4 font-mono text-sm border border-slate-700/80 shadow-inner overflow-x-auto leading-relaxed"
          >
            <span class="text-blue-300">&lt;component&nbsp;</span
            ><span class="text-purple-300">:is</span>=<span
              class="text-green-300"
              >"activeTab"</span
        ...
        >
          <p class="text-sm font-medium text-slate-500 text-center italic">
            (Notice we are actually building the interactive KeepAlive routing
            demo below using exactly this feature!)
          </p>
        </div>
      </div>
    </div>
    ...
    >
      ...
      >
        <h3 class="text-lg font-bold text-white flex items-center gap-2">
          <span class="text-orange-400">💾</span> KeepAlive
        </h3>
        <span
          class="text-xs font-mono px-2 py-1 rounded bg-orange-500/10 border border-orange-500/20 text-orange-400"
          >State / Routing</span
        >
      </div>
      <div class="p-6 md:flex gap-8 items-start">
        <div class="space-y-4 flex-1">
          <p class="text-slate-300">
            In React, when a component unmounts, its state is wiped unless
            explicitly cached in a global store (like Redux or Zustand). Vue's
            <code>&lt;KeepAlive&gt;</code> wrapper simply <b>pauses</b> a
            component's lifecycle.
          </p>
          <p class="text-slate-300">
            It is removed from the DOM, but its entire reactive instance tree is
            perfectly preserved in memory natively. Incredible for multi-step
            wizards or tab-based routing without complex global state!
          </p>

          <div
            class="bg-slate-900 rounded-lg p-4 font-mono text-sm border border-slate-700/80 shadow-inner"
          >
            <span class="text-blue-400">let</span> formDraft = ref(<span
              class="text-green-300"
              >""</span
            >); <br />
            <span class="text-slate-500"
              >// Preserved magically when swapping tabs</span
            >
          </div>
        </div>
        ...
        <!-- Demo -->
        <div
          class="flex-1 mt-6 md:mt-0 bg-slate-900 p-6 rounded-lg border border-slate-700/80 shadow-inner"
        >
          ...
          >
            <KeepAlive>
              <component
                :is="activeTab === 'A' ? TabAComponent : TabBComponent"
              />
            </KeepAlive>
          </div>
        </div>
    ...
    return () =>
      h("div", { class: "space-y-3 animate-in fade-in w-full text-left" }, [
        h(
          ...
          {
            class:
              "text-xs text-slate-400 font-bold uppercase tracking-widest flex items-center gap-2",
          },
          [
      ...
      h("div", { class: "space-y-3 animate-in fade-in w-full text-left" }, [
        h(
          ...
          [
            h("span", {
              class:
                "w-2 h-2 rounded-full bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.8)]",
            }),
            "Counter Instance",
          ],
        ),
