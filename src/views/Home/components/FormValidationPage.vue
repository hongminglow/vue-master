    ...
    >
      <template #demo>
        <div class="flex flex-col gap-4">
          ...
          >
            <div
              v-for="fieldKey in Object.keys(formData)"
              :key="fieldKey"
              class="flex flex-col gap-1.5"
            >
              <label
                class="capitalize font-bold text-slate-300 text-sm tracking-wide"
              >
                {{ fieldKey }}
              </label>
              <input
              ...
              <span
                ...
                class="text-xs font-medium text-red-400 mt-1 flex items-center gap-1"
              >
        ...
        <ul class="space-y-4">
          <li>
            <strong class="text-green-400 block mb-1"
              >🛡️ Universal Zod Ecosystem</strong
            >
            Zod handles schema validations equally perfectly across React and
            Vue. While React often ties it to `@hookform/resolvers`, Vue
            provides libraries that are structurally identical in purpose.
          </li>
          <li>
            <strong class="text-green-400 block mb-1"
              >📝 Native v-model Power</strong
            >
            Because of Vue's native two-way binding (`v-model`), you often don't
            strictly *need* a massive form library if you only want basic
            validation. You can simply bind inputs directly to a `reactive`
            object and test it against a Zod schema upon submit!
          </li>
          <li>
            <strong class="text-green-400 block mb-1"
              >🚀 Advanced Form Packages</strong
            >
            When you need complex state management (touched, dirty, submitting
            states, complex array paths), Vue offers extremely powerful packages
            mirroring React Hook Form. The two most prominent are
            `@tanstack/vue-form` (headless, built by Tanner Linsley's team
            alongside the React version) and `vee-validate` (extremely mature,
