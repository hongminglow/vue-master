    ...
    >
      <template #demo>
        ...
        >
          ...
          >
            <span
              class="text-xs absolute top-3 right-3 text-slate-500 font-mono tracking-wider font-bold"
            ...
            <button
              @click.stop="
                log(
                  'Inner Button clicked! 🛑 Vue automatically stopped propagation via `.stop`',
                )
              "
            ...
            <p class="text-xs text-slate-400 mt-4 leading-relaxed font-medium">
              ...
              <strong class="text-green-400">DOES NOT</strong> trigger the outer
              div's click tracker because we added the `.stop` modifier.
            </p>
          </div>
        ...
        <ul class="space-y-4">
          <li>
            <strong class="text-green-400 block mb-1"
              >🎭 Direct DOM vs Synthetic</strong
            >
            React binds events via JSX camelCase props: (`onClick`, `onSubmit`)
            and routes them through a massive global Synthetic Event root
            system. Vue attaches actual standard browser DOM Event Listeners
            mapped straight to the element via `v-on:` (or the shorthand `@`).
          </li>
          <li>
            <strong class="text-green-400 block mb-1"
              >🛠️ Powerful Inline Modifiers</strong
            >
            The absolute highest productivity feature in Vue templates. Instead
            of explicitly calling `e.stopPropagation()` inside a Javascript
            function, Vue provides powerful inline modifiers attached cleanly to
            the event via a dot suffix. `@click.stop` immediately handles
            bubbling natively.
          </li>
          <li>
            <strong class="text-green-400 block mb-1"
              >🛑 e.preventDefault() no more</strong
            >
            `e.preventDefault()` logic inside standard Javascript `onSubmit`
            handlers is completely eliminated by attaching the simplest
            modifier: `&lt;form @submit.prevent="submitForm"&gt;`.
          </li>
          <li>
            <strong class="text-green-400 block mb-1"
              >⌨️ Exact Key Tracking</strong
            >
            For keyboard events, you no longer need `if (e.key === 'Enter')`
            wrappers inside your logic block. You can specify exact tracking
            straight onto the HTML attribute: `@keyup.enter="trigger"` or
            `@keyup.ctrl.c="copy"`. Clean, beautiful, and completely decoupled
            from JS logic.
          </li>
        </ul>
      </template>
